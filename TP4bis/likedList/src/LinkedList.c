...
int ll_remove(LinkedList* this,int index)
{
	...
	if(this != NULL && index>=0 && index < ll_len(this)){
		...
		if(auxNode != NULL){
			if(index==0){
				//auxNodePrev = this->pFirstNode; //si es el primero, me guardo su dirección en NodePrev
				auxNodeNext = auxNode->pNextNode; //Guardo la dirección de su siguiente nodo en nodeNext
				this->pFirstNode = auxNodeNext; //Apunto el primer nodo de la LL a Next
				retorno = 0;
				this->size--;
			}else{
				auxNodePrev = getNode(this,index-1);
				if(auxNodePrev!=NULL){
					auxNodePrev->pNextNode = auxNode->pNextNode;
					retorno = 0;
					this->size--;
				}
			}
	...
	if (this != NULL && (order == 1 || order == 0) && pFunc != NULL) {
		for (int i = 0; i < ll_len(this) - 1; i++) {
			for (int j = i + 1; j < ll_len(this); j++) {
				...
				if (order == 1 && pFunc(auxElemento1, auxElemento2) > 0) {
					ll_set(this, i, auxElemento2);
					ll_set(this, j, auxElemento1);
				}
				if (order == 0 && pFunc(auxElemento1, auxElemento2) < 0) {
					ll_set(this, i, auxElemento2);
					ll_set(this, j, auxElemento1);
				}
			}
