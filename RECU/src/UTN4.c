...
int getPositIntPlus(int *pResultado, char* bufferNum) {
	/// @fn int getInt(int*)
	/// @param pNumeroTomado
	/// @return
	///
...
int myGetsLocal(char pResultado[], int length, char stringInput[]) {
	int retorno = -1;
	/// @fn int myGets(char[], int)
	/// @param pResultado
	/// @param length
	...
	if (pResultado != NULL && length > 0) {
		if (strncpy(bufferString,stringInput, length) != NULL) {
			if(strlen(stringInput)<length){
				strncpy(pResultado,bufferString,strlen(bufferString));
				retorno = 0;
				printf("pResultado es %s",pResultado);
			}else{
